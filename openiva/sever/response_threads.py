...
from threads.data import ThreadDataMulti2,ThreadFacialParse,ThreadCollect
from threads.multi import ThreadMulti2

from utils.pathes import RESNET50_365_MODEL_PATH_B1,YOLOV4_AD4_MODEL_PATH_B1,RESNET50_RETINAFACE_MODEL_PATH_B1,XCEPTION_28_MODEL_PATH_B1
from utils.pathes import RESNET50_365_MODEL_PATH_B4,YOLOV4_AD4_MODEL_PATH_B4,RESNET50_RETINAFACE_MODEL_PATH_B4,XCEPTION_28_MODEL_PATH_B4
from utils.pathes import RESNET50_365_MODEL_PATH_B8,YOLOV4_AD4_MODEL_PATH_B8,RESNET50_RETINAFACE_MODEL_PATH_B8,YOLOV4_OI_MODEL_PATH_B8,XCEPTION_28_MODEL_PATH_B8
...
# =============================================================================
    ...
    q_dict={'video_path':out_path,'task_id':str(uuid.uuid1()),'info_form':info_form,'padd':True}
    print(q_dict)
    Q_TASK.put(q_dict)
...
class ThreadCallback(Thread):
    def __init__(self,q_post):
        super(ThreadCallback,self).__init__()
        self.q_post=q_post
    
    def start(self):
        super(ThreadCallback,self).start()
...
def init_procs(batch_size=8,nb_proc=4,num_classes_yolo=4,path_names="/home/re02/pyprojects/VideoRecogInfer/datas/index/ad3.names"):    
    if batch_size==1:
        yolo_path=YOLOV4_AD4_MODEL_PATH_B1
        places365_path=XCEPTION_28_MODEL_PATH_B1
        retinaface_path=RESNET50_RETINAFACE_MODEL_PATH_B1
    elif batch_size==4:
        yolo_path=YOLOV4_AD4_MODEL_PATH_B4
        places365_path=XCEPTION_28_MODEL_PATH_B4
        retinaface_path=RESNET50_RETINAFACE_MODEL_PATH_B4
    elif batch_size==8:
        yolo_path=YOLOV4_AD4_MODEL_PATH_B8
        places365_path=XCEPTION_28_MODEL_PATH_B8
        retinaface_path=RESNET50_RETINAFACE_MODEL_PATH_B8
    
    arcface_path=RESNET50_ARCFACE_MODEL_PATH_B1

    model_configs={'yolov4':[yolo_path,(608,608),num_classes_yolo,'yolov4','yolov4'],\
                    'places365':[places365_path,(256,256),28,'places365','places365'],\
    ...
    th_compute=ThreadMulti2(q_compute_in,q_compute_out,q_facial_in,q_facial_out,model_configs,batch_size)

    th_collect=ThreadCollect(q_compute_out,q_post,class_names)
    th_facial=ThreadFacialParse(q_facial_in,q_facial_out)

    th_datas=[ThreadDataMulti2(q_task,q_compute_in,model_configs,batch_size=batch_size,skip=10) for i in range(nb_proc)]
    
    th_post=ThreadCallback(q_post)
