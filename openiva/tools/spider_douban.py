...
HEADERS_NORMAL={'accept':"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8","user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/69.0.3497.100 Safari/537.36"}
...
def _get_celebrities_list(url_subject):
    ...
    p_celeba=re.compile( r'<li class="celebrity">[\s\S]+?<div class="info">[\s\S]+?<a href="(.+?)" title="(.+?)"[\s\S]+?</div>')
...
def _format_headers(id_image,id_celeba):
    ...
    headers['referer']=headers['referer'].format(id_image=id_image,id_celeba=id_celeba)
    return headers
...
def _get_raw_photo_urls(id_celeba,max_img):
    url_root='https://movie.douban.com/celebrity/{}/photos/'.format(id_celeba)
    
    src_root=requests.get(url_root,headers=HEADERS_NORMAL).text
    turning_urls=_get_all_turning_url(src_root,max_img)

    src_all_photo=src_root+''.join([requests.get(url,headers=HEADERS_NORMAL).text for url in turning_urls])
    
    ids_image=_get_id_image(src_all_photo)
    urls_and_headers=[(_format_raw_url(iid),_format_headers(iid,id_celeba)) for iid in ids_image]
...
def _download(root_dir,raw_photo_url,headers):
    ...
    try:
        ...
        traceback.print_exc()
        print('Got error during download img \n{img}\n to file \n{fname}\nPass'.format(img=raw_photo_url,fname=raw_photo_url))
...
def worker_celeba(root_dir,celeba_tuple,max_img=100,overwrite=False):
    ...
    target_dir=os.path.join(root_dir,name+'_'+str(id_celeba)+'_'+gender_char)
    ...
    for raw_photo_url,headers in urls_and_headers[:max_img]:
        print('Downloading image of {name} from {url_img} ...'.format(name=name,url_img=raw_photo_url))
...
def crawl_celeba(root_dir,name,max_img=200,overwrite=False):
    ...
    url_celeba_search='https://movie.douban.com/celebrities/search?search_text={}'.format(name)
    ...
    if len(results)<1:
        raise SearchNoResult('Got no result when searching actor {}.'.format(name))
...
def crawl_cast_p(root_dir,url_subject,max_img=100,overwrite=False):
    ...
    for celeba_tuple in celebrities_list:
        mpool.apply_async(worker_celeba,(root_dir,celeba_tuple,max_img,overwrite))
    mpool.close()
    mpool.join()
...
if __name__=='__main__':
    ...
    crawl_cast_p('./datas/celebrity_images','https://movie.douban.com/subject/3742360',overwrite=True)
